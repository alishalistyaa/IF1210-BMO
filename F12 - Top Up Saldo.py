...
def topup_saldo():
    # Membaca user.csv dan menjadikan sebuah variabel
    data_user = CSV_Parser('user.csv')
        ...
        if data_user[i][1] == username_input:
            ...
            if int(saldo_topup) < 0:
                ...
                if (int(saldo_topup) + int(data_user[i][5])) >= 0:
                    print(f'Top up berhasil. Saldo {data_user[i][2]} bertambah menjadi {(int(data_user[i][5]) + int(saldo_topup))}.')
                    break
    ...
    if validasi != True :
        if data_user[i][1] != username_input :
            if saldo_topup != '' or saldo_topup == '':
                print(f'Username {username_input} tidak ditemukan')
